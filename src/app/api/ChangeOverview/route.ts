...
import { humanMessageForChanges, systemMessage } from "@/app/Workflow/prompts/GenerateOverview";
...
export async function GET(req: NextRequest) {
    try {
        ...
        const changesToMake = await req.nextUrl.searchParams.get("changesToMake");
        const currentResponse = await req.nextUrl.searchParams.get("currentResponse");

        const humanPrompt = await humanMessageForChanges.format({
            title: topic,
            timeline: duration,
