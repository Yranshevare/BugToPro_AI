...
export default function page({ params }: { params: Promise<{ task: string }> }) {
    ...
    useEffect(() => {
        ...
        (async function () {
            ...
            const { repoId, taskNumber } = JSON.parse(decodeURIComponent(task));
            try {
                const res = await secureReq({ url: "/api/Repo/GetOneTask", params: `repoId=${repoId}&taskNumber=${taskNumber}` });
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!answers[currentAssignment]) {
                return;
            }
            if (answers[currentAssignment].trim() === "") {
                return;
            }
            if (answers[currentAssignment].trim().toLowerCase() === task.assignments[currentAssignment].answer?.toLocaleLowerCase()) {
                return;
            }
            setSubmitting(true);
            ...
            const res = await axios.post(`/api/Repo/SubmitAns`, {
                repoId: (await params).task ? JSON.parse(decodeURIComponent((await params).task)).repoId : "",
                taskNumber: (await params).task ? JSON.parse(decodeURIComponent((await params).task)).taskNumber : "",
                answer: answers[currentAssignment],
                questionNo: currentAssignment,
                token: token
            });
            ...
            if (!res.data.data || !res.data.data.explanation) {
                return;
            }
            ...
            setTask((prevTask) => {
                ...
                return {
                    ...prevTask,
                    assignments: prevTask.assignments.map((asg, index) => {
    ...
    return (
        <div className="min-h-screen bg-gray-950">
            ...
            <div className="relative max-w-7xl mx-auto px-6 py-8">
                ...
                {/* Assignment Detail */}
                <div className={`grid ${currentAsg.explanation ? "lg:grid-cols-2" : "grid-cols-1"} gap-6 max-h-max-screen`}>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6">
                            ...
                            <textarea
                                ...
                                rows={16}
                                placeholder={`Write your answer here...\nExample:\n\nclass Solution {\n    public void solve() {\n        Your implementation\n    }\n}\nAbove code snippet is of Java program.`}
                                className="w-full bg-gray-900/70 border border-gray-700 rounded-xl px-4 py-3 text-gray-100 font-mono text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none mb-4 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-gray-800/50 [&::-webkit-scrollbar-thumb]:bg-gray-600/50"
                            />
                            <div className="flex gap-3">
                                <button
                                    disabled={submitting}
                                    onClick={handleSubmit}
                                    className={`${submitting ? "opacity-50 cursor-not-allowed" : ""} flex-1 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white py-3 rounded-xl font-semibold transition-all shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 flex items-center justify-center gap-2`}
                                >
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path
                                            strokeLinecap="round"
                                    ...
                                    {submitting ? "Analyzing Your Answer..." : currentAsg.answer ? "Update Answer" : "Submit Answer"}
                                </button>
                            </div>
                        </div>
                    </div>
